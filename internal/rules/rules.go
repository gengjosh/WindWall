...
func ShouldBlock(info RequestInfo) Result {
	if info.IsThirdParty {
		return Result{Allow: false, Reason: "Blocked: Third-party request"}
	}

	return Result{Allow: true, Reason: "Allowed: No blocking criteria met"}
}
